...
/*评论提交区结束*/
    ...
    $(".personnel").on('click', '.small_img', function(){ 
    	var This=$(this);
    	This.parent().after('<div class="bigImg"><img class="big_img" width="100%"/></div>');
    	This.parent().siblings('.bigImg').children().first().attr('src', $(this).attr('src'));
		...
		function ajax(r){
			...
			.done(function(d){
				if(d.showapi_res_error){
					var $laugh_div = $('<div class="laugh_fail" style="text-indent:10px">哎呀。。加载失败。</div>');
